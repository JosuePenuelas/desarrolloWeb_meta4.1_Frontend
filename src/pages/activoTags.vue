        ...
        <v-dialog v-model="showDeleteDialog" max-width="500px">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-text-field v-model.number="deleteTagId" label="Número de Inventario" type="number"></v-text-field>
                </v-card-text>
                <v-card-actions>
        ...
        <v-dialog v-model="showUpdateDialog" max-width="500px">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-text-field v-model.number="updateActivoId" label="Número de Serie" type="number"></v-text-field>
                    <v-text-field v-model.number="updateTagId" label="Número de Inventario"
...
async function cargarDatosUpdate() {
    try {
        const response = await fetch(`https://localhost:4000/activoTags/${updateId.value}`);
        const data = await response.json();
        updateActivoId.value = data.activoId;
        updateTagId.value = data.numInv;
    } catch (error) {
